...
引数には生成したNoteControllerクラスの参照が取得される。
  ...
  説明：ノーツの透明度(0～1)を設定。これも **使用頻度ほぼ0**。確かUnityでの透過処理はアプリの動作が重くなる原因だった気がするので多用厳禁。
...
<details>
<summary>通常のロングの各Z座標の位置計算</summary>

ロングの各Z座標の位置は次の３段階に分けて計算しています
...
使用しているAnimationCurveはLuaからも CS.GameManager.Instance.NoteScrollAnimationCurveで取得可能です。
  ...
  <details>
  <summary>SetWidthScale(float width)</summary>

  戻り値：なし

  引数：float 幅(0～1)
  
  説明：ロングの幅比を設定...する筈っだったのだがエラーが出てしまい、唯一無二の「LuaAPIの仕様」から削除された関数である。どうやらまだ使えるみたいだが、使用しないことを強く推奨する。

  ```lua
  longController:SetWidthScale(0.5)
  ```
  </details>
